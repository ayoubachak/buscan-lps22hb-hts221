...
void send_lum_can(void)
{
    ...
    if (status) {
       // SetDisplayString("Er 4");
    	...
    	msg_test.id =0x09;
    	msg_test.data[0] = (uint8_t)((Als.lux & 0xFF) );
    	msg_test.data[1] = (uint8_t)(Als.lux >> 8);
...
void RangeState(void) {
    ...
    if (Range.errorStatus) {
    	...
    	CAN_Message msg_test;
		msg_test.id =0x05;
		msg_test.data[0] = (uint8_t)((range & 0xFF) );
		msg_test.data[1] = (uint8_t)(range >> 8);
		msg_test.len=2;			// Nombre d'octets à envoyer
		msg_test.format=CANStandard;
		msg_test.type=CANData;
